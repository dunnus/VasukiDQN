...
class Vasuki(Env):
    ...
    def _init_image_(self, path):
        ...
        # Resizing the image
        image = cv2.resize(
            image, (self.scale - 1, self.scale - 1), interpolation=cv2.INTER_NEAREST
        )
    ...
    def __init__(self, n, rewards, game_length=100):
        ...
        self.live_index = np.random.choice(
            len(self.foodspawn_space), size=(self.n // 2), replace=False
        )
        ...
        # {"agentA": self.agentA, "agentB":self.agentB, "live_foodspawn_space": self.live_foodspawn_space, 'encoded': encoded}
        self.history = []
    ...
    def _reward_(self, agent, illegal):
        ...
        if illegal == 1:  # If the snake hits the wall
            ...
            if True in np.all((state == self.live_foodspawn_space), axis=1):
                ...
                index = np.where(
                    np.all((state == self.live_foodspawn_space), axis=1) == True
                )[0].item()
                # Computing the empty foodspawn spaces
                empty_foodspawn_space = [
                    space
                    for space in self.foodspawn_space
                    if space not in self.live_foodspawn_space
                ]
                # Removing the state from live foodspawn space
                self.live_foodspawn_space = np.delete(
                    self.live_foodspawn_space, index, 0
                )
                # Updating the live foodspawn space
                addition = np.random.choice(
                    len(empty_foodspawn_space), size=1, replace=False
                )
                self.live_foodspawn_space = np.append(
                    self.live_foodspawn_space,
    ...
    def step(self, action):
        ...
        if (self.agentA["state"] == self.agentB["state"]).all():
            if self.agentA["score"] > self.agentB["score"]:
                rewardA = 5 * abs(
                    self.agentB["score"]
                    // (self.agentA["score"] - self.agentB["score"])
                )
                rewardB = -3 * abs(
                    self.agentB["score"]
                    // (self.agentA["score"] - self.agentB["score"])
        ...
        self.history.append(
            {
                "agentA": self.agentA,
                "agentB": self.agentB,
                "live_foodspawn_space": self.live_foodspawn_space,
                "encoded": encoded,
                "rewardA": rewardA,
                "actionA": actionA,
                "rewardB": rewardB,
    ...
    def render(self, actionA, actionB):  # Returns a one-hot encoded state
        ...
        for x in range(self.n + 1):
            state[self.scale * x : self.scale * x + 1, : self.scale * self.n] = [
                0,
                0,
                0,
            ]
        ...
        for food in live_foodspawn_space_.tolist():
            ...
            y = int(food[1])
            state[
                self.scale * x + 1 : self.scale * x + self.scale,
                self.scale * y + 1 : self.scale * y + self.scale,
            ] = self.image_prey
        # Annotating
        ...
        start = 80
        state = cv2.putText(
            state, scoreA, (265, start), font, fontScale, color, thickness, cv2.LINE_AA
        )
        state = cv2.putText(
            state,
            stateA,
        ...
        state = cv2.putText(
            state, scoreB, (390, start), font, fontScale, color, thickness, cv2.LINE_AA
        )
        state = cv2.putText(
            state,
            stateB,
        ...
        state[
            self.scale * snakeA[0] + 1 : self.scale * snakeA[0] + self.scale,
            self.scale * snakeA[1] + 1 : self.scale * snakeA[1] + self.scale,
        ] = image_agentA
        state[
            self.scale * snakeB[0] + 1 : self.scale * snakeB[0] + self.scale,
            self.scale * snakeB[1] + 1 : self.scale * snakeB[1] + self.scale,
        ] = image_agentB
        # Returning the state
        return state
    ...
    def encode(self):
        ...
        state[live_foodspawn_space[:, 0], live_foodspawn_space[:, 1]] = encoder[
            "foodspawn_space"
        ]
